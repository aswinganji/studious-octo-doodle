...
def bike_control():
    dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.55))
    time.sleep(15)
    dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.2,steer = -0.35))
    time.sleep(10)
    dropped_vehicle.set_light_state(carla.VehicleLightState(carla.VehicleLightState.Brake |
                                                                        carla.VehicleLightState.LeftBlinker | carla.VehicleLightState.HighBeam))
    time.sleep(1.0)
...
def semantic_lidar_data(point_cloud_data):
    ...
    for detection in point_cloud_data:
        ...
        for object_name in object_id:
            if distance_name_data['name'] == object_name and distance_name_data["distance"] > 5 and distance_name_data[
                "distance"] < 7:
                dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.31))
                if distance_name_data['name'] == object_name and distance_name_data["distance"] > 3 and distance_name_data[
                "distance"] < 6:
